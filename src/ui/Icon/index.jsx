...
const Icon = ({ children, counter, name }) => {
  ...
  const [state, dispatch] = useReducer(reducer, { accountHover: false, cartHover: false })
  return (
    <div onMouseEnter={(!counter && onHover) || (name === "Cart" && onCartHover)} onMouseLeave={(!counter && onHover) || (name === "Cart" && onCartHover)} className={styles.icon}>
